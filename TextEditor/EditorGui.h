...
public:
	...
	// dictionary: The fill path and filename of the dictionary.txt file
...
private:
	...
	// Process each key that the user presses and call the appropriate function in the
	// editor class.
	...
	bool processKey(const int ch) {
		...
		case KEY_ENTER:
			te_->enter();
			break;
		case CTRL_S:	// fin the current changes
			save();
			return true;
	...
	std::string getSuggestionString() {
		...
		// Ask the spell checker if the word is spelled correctly, and if not
	...
	void redisplayTheEditorWindowAndPositionCursor(bool clear_status_line = true) {
		...
		// Obtain the relevant lines from the Text Editor class and display them
